...
{ 
	...
	// 对应的那两位配置为0000，INPUT
	(*(volatile unsigned int *)(pio_port_base_addr)) = ((*(volatile unsigned int *)(pio_port_base_addr)) & ~(0x000f<<((port_bit-con_state)*4)))|(0x0000<<((port_bit-con_state)*4));

 //00 = pull-up/down disabled   01 = pull-down enabled  10 = pull-up enabled  11 = Reserved.
	(*(volatile unsigned int *)(pio_port_base_addr+S3C6410_GPXPUD_OFFSET))  = ((*(volatile unsigned int *)(pio_port_base_addr+S3C6410_GPXPUD_OFFSET))  & ~(0x0f<< (port_bit*2)));

	val = (*(volatile unsigned int *)(pio_port_base_addr+S3C6410_GPXDAT_OFFSET)) & (0x0001<<port_bit);
...
//	ADC_BaseVirt =(unsigned int *) (gpioports + 0);
